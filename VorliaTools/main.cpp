...
int main()
{
	...
	printf("Vorlia tools 1.3\n");
	printf("================\n");
	printf("(c) Vorlia Team\n");
...
int CToolsClass::execute_sub_command()
{
	...
	{
		printf("==== Vorlia tools == accounts received from the server ========================\n");
	...
	{
		printf("==== Vorlia tools == characters received from the server ========================\n");
...
//				else
		...
		if (requestwhat == 4)
		{
			...
			printf("requesting acccount info %s from the server\n", tosend);
...
//	tmptime = SDL_GetTicks() - conn_time;
	...
	if (secs < 10) sprintf(sec, "0%d", secs);
	else sprintf(sec, "%d", secs);
	if (mins < 10) sprintf(min, "0%d", mins);
	else sprintf(min, "%d", mins);
	if (hours < 10) sprintf(hour, "0%d", hours);
	else sprintf(hour, "%d", hours);

	sprintf(time_, "%d:%d:%d", hour, min, sec);
}
