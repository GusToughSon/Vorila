...
{ /* Heavily modified by Jussi Witick, 19.9.2005 */
    int x = 0;

	int len = strlen(text);
	for (int i = 0; i < len; i++)
	{
		if (text[i] != 'ô' && text[i] != 'Æ' && text[i] != 'æ' && text[i] != 'É' && text[i] != 'Ä' && text[i] != 'Å' && text[i] != 'ì' && text[i] != 'î' && text[i] != 'ï')
		x += BFont_CharWidth(CurrentFont, text[i]);
	}
    return x;
}
...
/* Following function has been coded by Jussi Witick. */
void Print_ChatMessage(SDL_Surface *Surface, BFont_Info *Font_1, BFont_Info *Font_2, int x, int y, char *name, char *msg)
{
    while ( *name ) 
	{
        x  += BFont_PutCharFont(Surface,Font_1,x,y, *name);
        name++;;
    }
	...
	while ( *msg )
	{
		x += BFont_PutCharFont(Surface, Font_2, x, y, *msg);
		msg++;;
	}
}