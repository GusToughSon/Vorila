...
void DrawClass::DrawFPS()
{
	...
	// Let's draw ORBs if player has any.
...
//		BFont_PrintStringFont(Screen.screen, F.white, 764, 14, "%d", P.StatPoints);
	...
	// Let's draw little x into right upper corner.
	...
	// Let's draw Exp-o-meter and Alignmentbar here
...
/*
	...
	for (i1 = 0; i1 < MaxNPC; i1++)
	{
		if (NPC[i1].Used)
		{
			x1 = -pldatabase->own->x + NPC[i1].X + 10;
			y1 = -pldatabase->own->y + NPC[i1].Y + 10;

			if (MapInfo.Visible[-pldatabase->own->x + NPC[i1].X + 10][-pldatabase->own->y + NPC[i1].Y + 10])
			{
				Draw.PrintReputationText(x1 * 20 + 17, y1 * 20, 0, 5, NPCTypes[NPC[i1].Type].Name, 0, 0);
			}
		}	
	}*/
}
...
void MessageBoxClass::NewMessage(char msg[180], short MsgType)
{
	strcpy(NewMsg, msg);
    NewType = MsgType;
	Update();
...
void LoginClass::DrawTexts()
{
	...
	else
	{
		...
		{
			if (Writing.Mode == 11)
				sdl.text("*", 398 + i * 6, 243, col_grey);
			else
				sdl.text("*", 398 + i * 6, 243, col_gray);
		}
		if (Writing.Mode == 11)
			sdl.text("|", 398 + Writing.LoginHandler[1]*6, 243, col_white);
	}
}
