...
void CheckAutoTarget()
{
	...
	while (mon)
	{
		if (mon->x == autotX + pldatabase->own->x - 10 && mon->y == autotY + pldatabase->own->y - 10 && mon->knowid != Target.TarID)
		{
...
//			Shop.BuyGuild = true;
	...
	case 60:
		...
		case 2: // Invisibility on/off
			...
			if (Invisibility == 1)	
				Info->newInfoMsg("You are now invisible");
			if (Invisibility == 0)
				Info->newInfoMsg("You are now visible");
		...
		case 4: // ListenNormal on/off
			...
			if (ListenNormal == 1)			
				Info->newInfoMsg("Listening worldwide normal chat");
			if (ListenNormal == 0)
				Info->newInfoMsg("Not listening worldwide normal chat anymore");
	...
	case 99: 
		...
		case 30:
			...
			Info->newInfoMsg("Character name is already taken, can't change!");

			break;

		case 31:

			Info->newInfoMsg("Character name changed!");
...
void LoginClass::PrepareAccCreation()
{
	...
	textedits->NewTextedit("", 395, 208, 150, 16, false, false, 20, false, false, 1000001, 1000002);
	textedits->NewTextedit("", 395, 240, 150, 16, false, false, 20, false, false, 1000002, 1000001);
