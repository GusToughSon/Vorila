...
int main()
{
	...
	CreateDirectory("GM personal logs", NULL);
	...
	TimeoutandSaveTime	= GetTickCount();
	AttackCheck			= GetTickCount();
	Autosavetime		= GetTickCount();
	ReplaceOreTime		= GetTickCount();
	...
	while (!Done)
	{
		...
		if (GetTickCount() - TimeCheckTime > 50)
		{
		    _strtime( Time );
			_strdate( Date );
			TimeCheckTime = GetTickCount();
		}
		...
		if (GetTickCount() - LastDotTime > 10)
		{
			Check_Connections();
			LastDotTime = GetTickCount();
		}
		...
		if (GetTickCount() - TimeoutandSaveTime > 3000)
    {
			int i, j;
			...
			for (i = 0; i < Max_Connections; i++)
			{
				...
				if (Clients[i].Used)
				{
					if (GetTickCount() - Clients[i].LstMsg > 15000)
					{
		...
		if (GetTickCount() - MonsterActionTime > MonsterActionCheckTime)
		{
				...
				for (mon_loopj = 0; mon_loopj < 128; mon_loopj++)
        {
					...
					while (mon_List != NULL)
					{
						...
						if (GetTickCount() - mon_List->LstReg >= 4000)
					...
					while (npclooplist != NULL)
					{
						...
						if (NPCTypes[ npclooplist->Type ].is_guard)
						{
							...
							{
								if (NPCTypes[ npclooplist->Type ].is_guard && GetTickCount() - npclooplist->LstAttack > 15000)
						...
						{
							if (GetTickCount() - npclooplist->LstMove > 1800 - rand() % 1600)
							{
		...
		if (GetTickCount() - LastSpawnTime > MonsterSpawnTimeCheck)
		{
			...
			for (i = 0; i < MaxItems; i++)
			{
				if (Ground[i] > 0)
				{
					if (GetTickCount() - GroundTime[i] > 180000)
					{
		...
		if (GetTickCount() - PlayerActionTime > PlayerActionTimeCheck)
		{
			int i;
			for (i = 0; i < Max_Connections; i++)
			{
				if (Clients[i].Used && Clients[i].State != 5)
				{
					...
					{
						if (GetTickCount() - Clients[i].LstJailtime >= 60000)
						{
							...
							Clients[i].LstJailtime = GetTickCount();
						}
					}
					if (Clients[i].AttackTarget == 0 && Clients[i].PLTarget != -1)
					{
						...
						{
							if (GetTickCount() - Clients[i].LstAtk >= Weapons[Items[Clients[i].Weapon].Type][Items[Clients[i].Weapon].ID].Speed) //- Skill)
								CanAttack = true;
						}
						if (Clients[i].Weapon == 0)
						{
							if (GetTickCount() - Clients[i].LstAtk >= 1400) //- Skill)
								CanAttack = true;
						}
					...
					if (GetTickCount() - Clients[i].LstReg >= 4000 && Clients[i].uChar->HPLeft < Clients[i].Acc->Calc_hpMax())
					{
						// Regenerate HP
						short Damage;
						...
						Clients[i].LstReg = GetTickCount();	
					}
					if (GetTickCount() - Clients[i].lst_tradeskill > 3000 && Clients[i].tradeskill_inuse)
					{
						Clients[i].use_skill();
					}
				}
		...
		if (GetTickCount() - CheckObjectsTime > 500)
		{
			...
			for (i = 0; i < 25; i++)
			{
				if (Objects_Use[i])
				{
					if (GetTickCount() - Objects_UseTime[i] > 2000 && UseTargets[i]->Type == 1)
					{
						if (!UseTargets[i]->deuse(-1))
						{
							Objects_UseTime[i] = GetTickCount();
						}
						else
						{
							Objects_Use[i] = false;
			...
			for (i = 0; i < m_Bodies; i++)
			{
				if (Body[i].Used)
				{
					if (GetTickCount() - Body[i].DecayStateTime > body_DecayTime)
					{
						Body[i].DecayState++;
			...
			for (i = 0; i < 10; i++)
			{
				if (SecureTrade[i].TradeInPending && !SecureTrade[i].TradeInProgress)
					if (GetTickCount() - SecureTrade[i].LastAction > 4000)
						SecureTrade[i].Abort();

				if (!SecureTrade[i].TradeInPending && SecureTrade[i].TradeInProgress)
					if (GetTickCount() - SecureTrade[i].LastAction > 20000)
						SecureTrade[i].Abort();
			}				

			CheckObjectsTime = GetTickCount(); 
		}

		// Replace mountains with ores.
		if (GetTickCount() - ReplaceOreTime > 600000)
		{
...
void Get_Message( short Player)
{
	...
	{
		Clients[Player].LstMsg = GetTickCount();
	}
	...
	case 3: //  Char chosen
		...
		{
			Clients[Player].LstJailtime = GetTickCount();
...
//		Clients[Player].ReportStats();
		...
		sprintf(Msg, "Welcome to the lands of Eldoria!");
...
//					send(Clients[i].ClientSocket, buffer, 2 + sizeof(NewPacket), 0);
	...
	case 10: 
		...
		if (GetTickCount() - Clients[Player].lst_tradeskill < 3000)
			break;

		if (Clients[Player].uChar->DevMode >= 2)
		{
			if (GetTickCount() - Clients[Player].LstMov < 50)
				break;
		}

		if (!Clients[Player].GMMode && Clients[Player].uChar->DevMode < 2)
		{
			if (GetTickCount() - Clients[Player].LstMov < 150)
				break;
		}
		...
		{
			if (GetTickCount() - Clients[Player].LstMov < 550)
				break;
		}

		Clients[Player].LstMov = GetTickCount();
			...
			if (ItemMove.ToList == ItemGround)
			{
				...
				if (ItemMove.Amount == 0)
				{
					if (IsValid(ItemMove.x, ItemMove.y))
					{
						...
						Items[This].x = ItemMove.x;
						Items[This].y = ItemMove.y;
						GroundTime[ItemMove.ItemID] = GetTickCount();
					}
					break;
				}
			}
	...
	case 21: // Potions
		...
		{
			if (GetTickCount() - Clients[Player].LstDeath < 2000) // I'm using LstDeath as timer in using potions, since deathtimer isn't operational.
			{
				buffer[0] = 99;
				buffer[1] = 14;
			...
			if (Clients[Player].uChar->HPLeft < Clients[Player].Acc->Calc_hpMax())
			{
				...
				{
					Clients[Player].LstDeath = GetTickCount();
					short Damage;
	...
	case 30: // Player targetted something.
		...
		case 1: // Target is monster
			...
			if (Clients[Player].MonTarget != NULL)
			{
				...
				Clients[Player].LstAtk = GetTickCount();			
			}
...
//							else
	...
	case 54: // Secure trade
		...
		case 2: // Trade is unlocked
			...
			SecureTrade[curtrade].LastAction = GetTickCount();
			short t;
		...
		case 3: // Offer trade to someone
			...
			SecureTrade[FreeTrade].LastAction = GetTickCount();

			SecureTrade[FreeTrade].TradeInPending = true;
			SecureTrade[FreeTrade].Traders[0] = Player;
//			SecureTrade[FreeTrade].PendingTime = GetTickCount();
		...
		case 4: // Someone accepted a trade offered to him
			...
			for (i = 0; i < 10; i++)
			{
				if (SecureTrade[i].Traders[1] == Player && SecureTrade[i].TradeInPending)
				{
					...
					SecureTrade[i].LastAction = GetTickCount();
...
//					memset(&SecureTrade[i].tradeitems, -1, sizeof(SecureTrade[i].tradeitems));
	...
	case 55: // Mining
			...
			Items[Clients[Player].uChar->Backpack[MineAt.WithItem].ID].Type == TYPE_USEABLEITEM)
		{
			...
			Clients[Player].lst_tradeskill = GetTickCount();
	...
	case 60: // GM Things................just remember to check character's DevMode -> nobody can hack this.
		recv(Clients[Player].ClientSocket, buffer, 1, 0);
...
/*
	case 125: // Eldoria Tools things
		...
		case 4: // Update account on the server
			...
			printf("Eldoria tools: Account %s updated\n", accToEdit.AccName);
...
//			recv(Clients[Player].ClientSocket, buffer, sizeof(message), 0);
		...
		case 11: // auth
			...
			printf("Eldoria tools trying to auth\n");
			...
			for (i = 0 ; i < 3; i++)
			{
				if (!strcmp(auth[i].name, name))
				{
					...
					{
						printf("Eldoria tools authed from ip %s\n", ClientIp);
...
/////////////////
	case 120: // Disconnect
		...
		if (buffer[0] == 1) // Check for safe zone
			Clients[Player].LstLogTime = GetTickCount() + 11000; // Set the logout timer
		else			
			Clients[Player].LstLogTime = GetTickCount(); // Set the logout timer
		...
		if (Clients[Player].tradeskill_inuse == false) // Make sure they are not tradeskilling
		{
			...
			{
				if (GetTickCount() - Clients[Player].LstLogTime >= 10000) // Check if timer is 10 seconds or more
				{
					buffer[0] = 11;
					buffer[1] = 1;
					memcpy(&buffer[2], &accID, sizeof(accID));
				...
				Clients[Player].LstLogTime = GetTickCount();
			}
		}
