...
void convert_graphics();
void convert_logingraphs();
void convert_staticgraphs();
void convert_tilegraph();
void convert_itemgraph();
void convert_avatarsgraph();
void convert_addgfgraph();
void convert_objectsgraph();	
void convert_gdata7();
...
{
	convert_logingraphs();
	convert_staticgraphs();
	convert_tilegraph();
	convert_itemgraph();
	convert_avatarsgraph();
	convert_addgfgraph();
	convert_objectsgraph();	
...
{
	SDL_Surface *tmp;
	printf("starting to convert addgf graphics...\n");

	long it;

	ofstream fout("gfdata for client/gdata05.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening addgf.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/addgf.bmp");
	it = tmp->h; it = it << 2;
...
{
	SDL_Surface *tmp;
	printf("starting to convert object graphics...\n");

	long it;

	ofstream fout("gfdata for client/gdata06.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening objects.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/objects.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
{
	SDL_Surface *tmp;
	printf("starting to convert avatars graphics...\n");

	long it;

	ofstream fout("gfdata for client/gdata04.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening avatars.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/avatars.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
{
	SDL_Surface *tmp;
	printf("starting to convert item graphics...\n");

	long it;

	ofstream fout("gfdata for client/gdata03.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening itemgraph.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/itemgraph.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
{
	SDL_Surface *tmp;
	printf("starting to convert tile graphics...\n");

	long it;


	ofstream fout("gfdata for client/gdata02.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening tiles.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/tiles.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
{
	SDL_Surface *tmp;
	printf("starting to convert login graphics...\n");

	long it;


	ofstream fout("gfdata for client/gdata00.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening acccreation.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/acccreation.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening bg1.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/bg1.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening charchoose.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/charchoose.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening charcreation.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/charcreation.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening login.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/login.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening main.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/main.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
{
	SDL_Surface *tmp;
	printf("starting to convert static graphics...\n");

	long it;


	ofstream fout("gfdata for client/gdata01.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening identify.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/identify.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening layout.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/layout.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening List.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/List.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening mouse.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/mouse.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening shop.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/shop.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
	...
	printf("opening skills.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/skills.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
void convert_gdata7()
{
	...
	ofstream fout("gfdata for client/gdata07.vdf", ios::binary);
	if (!fout.is_open()) 
	{
	...
	printf("opening mbexp.bmp..");
	tmp = SDL_LoadBMP("gf_bmp/mbexp.bmp");
	it = tmp->h; it = it << 2;
	fout.write((char *)&it, sizeof(it));
	it = rand() % 10000000;
	fout.write((char *)&it, sizeof(it));
...
void convert_items()
{
	...
	for (int i = 0; i < m_Item_Types; i++)
	{
		for (int j = 0; j < OLD_m_Items_per_type; j++)
		{
			...
			strcpy(Weapons[i][j].Name, OLD_Weapons[i][j].Name);
			//================
...
//			Armors[i][j].Speed = OLD_Armors[i][j].Speed;			
			...
			strcpy(Armors[i][j].Name, OLD_Armors[i][j].Name);
			//================

		}
	}

	for (i = 0; i < OLD_m_Items_per_type; i++)
	{
		...
		strcpy(Useables[i].Name, OLD_Useables[i].Name);
...
//		Collectables[i].MaxDurability = OLD_Collectables[i].MaxDurability;
		...
		strcpy(Collectables[i].Name, OLD_Collectables[i].Name);
	}
	...
	for (i = 0; i < sizeof(Collectables); i++)
	{
		data[i] ^= 212;	
	}
...
//		memcpy(&NPCTypes[i].Loot, &OLD_NPCTypes[i].Loot, sizeof(NPCTypes[i].Loot));
		...
		strcpy(NPCTypes[i].Name, OLD_NPCTypes[i].Name);
	...
	for (i = 0; i < sizeof(MonsterTypes); i++)
	{
		data[i] ^= 212;	
	}
